/**
 * AssetPreview.jsx — renders ONE asset in its own canvas, in real-world scale.
 *
 ...
 * silhouette gives instant scale reference for the VR walk-through review.
 *
 * Two modes:
 *   • thumbnail  (interactive=false) — one static render, fixed 3/4 angle, cheap.
...
function PropModel({ assetKey, M, animate }) {
  ...
  return <primitive ref={ref} object={obj} userData={{ size }} />
}
...
function FitCamera({ assetKey, M, interactive }) {
  ...
  useEffect(() => {
    const o = buildProp(assetKey, M); o.scale.setScalar(PROP_SCALE); o.updateMatrixWorld(true)
